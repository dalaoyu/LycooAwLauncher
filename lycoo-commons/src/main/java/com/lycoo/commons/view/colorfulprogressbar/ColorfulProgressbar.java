package com.lycoo.commons.view.colorfulprogressbar;
...
public class ColorfulProgressbar extends ViewGroup {
    ...
    private void getParameter(Context context, AttributeSet attrs){
        if(attrs!=null) {
            ...
            style = ta.getString(R.styleable.ColorfulProgressbar_styleBar);
            if (!STYLE_NORMAL.equals(style) && !STYLE_COLORFUL.equals(style)) {
                style = STYLE_COLORFUL;  //如果没有在xml中显示设置style，默认使用双色进度条
            }
            progress = ta.getInteger(R.styleable.ColorfulProgressbar_progressBar, (int)progress);
            secondProgress = ta.getInteger(R.styleable.ColorfulProgressbar_secondProgressBar,(int)secondProgress);
            maxProgress = ta.getInteger(R.styleable.ColorfulProgressbar_maxBar, (int) maxProgress);
            backgroundColor = ta.getColor(R.styleable.ColorfulProgressbar_backgroundColorBar, backgroundColor);

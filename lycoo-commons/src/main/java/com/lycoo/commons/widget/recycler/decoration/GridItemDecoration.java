package com.lycoo.commons.widget.recycler.decoration;
...
public class GridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void drawHorizontal(Canvas c, RecyclerView parent) {
        ...
        CustomRecyclerViewAdapter adapter = (CustomRecyclerViewAdapter) parent.getAdapter();
    ...
    private boolean isLastRaw(RecyclerView parent, int pos, int spanCount, int childCount) {
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ...
            return (pos - adapter.getHeaderViewsCount() + 1) > leftCount;
        }
        return false;
    }
    ...
    private boolean isLastColumn(RecyclerView parent, int pos, int spanCount) {
        ...
        CustomRecyclerViewAdapter adapter = (CustomRecyclerViewAdapter) parent.getAdapter();
        if (layoutManager instanceof GridLayoutManager) {
            return (pos - adapter.getHeaderViewsCount()) % spanCount == 0;
        }
        return false;
    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (adapter.isFooter(itemPosition) || adapter.isHeader(itemPosition) || adapter.isRefreshHeader(itemPosition)) {
            ...
            if (!(parent.getLayoutManager() instanceof GridLayoutManager)) {
                //LinearLayoutManager
                if (itemPosition == (childCount - 2 - adapter.getHeaderViewsCount()))
                    outRect.set(0, 0, 0, 0);
                else
                    outRect.set(0, 0, 0, verticalSpace);
            } else {
                //GridLayoutManager
                if (isLastRaw(parent, itemPosition, spanCount, childCount - 2 - adapter.getHeaderViewsCount())) {
                    //最后一行
